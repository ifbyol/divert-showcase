...
async function callDownstreamService(headers) {
  ...
  const serviceUrl = `http://servicecv:8080/chain`;
...
app.get("/chain", async function (req, res) {
  ...
  try {
    ...
    const message = `Service CU says hello from ${process.env.OKTETO_NAMESPACE}! <br />`;
...
app.listen(PORT, function () {
  console.log("Started service-cu server on %d", PORT);
});